...
class Solution {
  ...
  Node bstFromLevel(int arr[], int n) {
    ...
    //index start from 1 because of root set
    int idx = 1;
    //making queue
    ...
    while (q.size() > 0) {
      ...
      //if there are more element left
      ...
      //i can also be potential parent, so for that i will need to add left and right platforms
