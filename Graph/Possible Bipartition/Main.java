...
class Solution {
  ...
  public class Pair {
    ...
    int node;
    int color; //1:blue color & 2:red color
  ...
  public int possibleBipartition(int n, int[][] dislikes) {
    ...
    for (int i = 1; i <= n; i++) {
      //this loop helps us to apply bfs on every component
      ...
      while (q.size() > 0) {
        ...
        for (int nbr : graph.get(curr.node)) {
          if (visited[nbr] == 0) {
            //when the nbr is colorless
            q.add(new Pair(nbr, oppColor));
          } else if (visited[nbr] == curr.color) {
            return 0;
            //not bipartate when neigbour is having the same color as us

          } else {
            continue; //nbr is having oppColor already
          }
        }
      }
    }
...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < M; i++) {
      for (int j = 0; j < 2; j++) dislike[i][j] = sc.nextInt();
    }
